...
class YDL:
    ...
    def __init__(
        self, progress_hook=None, logger: Optional[Logger] = None, quiet: bool = False
    ):
        self.ydl_opts: dict = {
            "quiet": quiet,
    ...
    def generate_ydl_opts(self, ext: str, ext_quality=None) -> dict:
        ...
        if ext in FORMAT_EXTS["video"]:
            try:
                ...
                raise QualityTypeError(
                    'Failed to determine the "quality" type. Expected:', *VIDEO_QUALITY
                )

            ydl_opts[
                "format"
            ] = f"bestvideo[height<={ext_quality}]+bestaudio/bestvideo[height<={ext_quality}]/best"
            ...
            ydl_opts["postprocessors"].append(
                {
                    ...
                    "preferredcodec": ext,
                    "preferredquality": ext_quality,
                }
            )

        else:
            raise ExtTypeError(
                'Failed to determine the "ext" type. Expected:',
                FORMAT_EXTS["video"],
                FORMAT_EXTS["audio"],
            )
    ...
    def search_multiple(self, query, search_provider: str) -> Any:
        with YoutubeDL(params=self.ydl_opts) as ydl:
            if search_provider:
                info = ydl.extract_info(f"{search_provider}:{query}", download=False)
                return info
            else:
                raise ValueError(
